...
#; (in-range 200)
#; (for/list ([i (in-range 10000000000000000000000000000000000000000000000)]
           [j (list 1 2 3 4 5)])
  i)
#; (for/list ([i 10000000000000000000000000000000000000000000000]
           [j (list 1 2 3 4 5)])
  i)
#; (in-naturals 10)
#;(for/sum ([i (in-range 2000000000000000)]
      [j (in-naturals 100)])
  (+ i j))
...
(define A-LIST (build-list 1000000 id))
...
#; (time (foldl + 0 (build-list LARGE-COUNT id)))
#; (time (stream-foldl + 0 (stream-take LARGE-COUNT (numbers 0))))
...
(define (evens n)
  (if (odd? n)
      (make-stream (add1 n) (lambda () (evens (add1 n))))
      (make-stream (+ 2 n) (lambda () (evens (+ 2 n))))))

(stream->list (stream-take 20 (evens 10)))
