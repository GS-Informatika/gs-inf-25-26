...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname 16-interactive-programs) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
; changes state after time-remaining ticks.
(define-struct world-state [tl time-remaining])

; TrafficLightState -> WorldState
; initial WorldState for given light state
(define (tl-with-initial-duration state)
  (make-world-state state (tl-state-duration state)))

; WorldState -> Boolean
; determines if the light should be
; advanced to the next state
(define (tl-should-switch state)
  (= (world-state-time-remaining state) 0))

; WorldState -> WorldState
; provides world state in next tick
(define (tl-advance-tick state)
...
(define (tl-tick-handler state)
  (if (tl-should-switch state)
      (tl-with-initial-duration (transition-tl (world-state-tl state)))
      (tl-advance-tick state)))
